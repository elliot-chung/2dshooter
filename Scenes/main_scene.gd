...
func _spawn_enemies():
	...
	for _i in range(n_to_spawn):
		...
		enemy.connect("enemy_died",Callable(self,"_handle_enemy_death"))
		enemy.connect("enemy_health_changed", Callable(Player, "handle_enemy_health_change"))
		live_enemies += 1
...
func _process(delta):
	...
	if !round_concluded && spawn_queue.size() == 0 && live_enemies == 0:
		get_tree().call_group("eor_clear", "queue_free")
